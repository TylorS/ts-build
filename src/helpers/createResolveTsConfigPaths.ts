...
export type TsConfigPathsResolver = {
  readonly resolvePath: (moduleSpecifier: string) => Promise<Option<string>>
...
export function createResolveTsConfigPaths({
  ...
  const matchPath = canMatchPath ? createMatchPathAsync(baseUrl!, paths, Array.from(mainFields)) : null

  const resolvePath: (moduleSpecifier: string) => Promise<Option<string>> = matchPath
    ? (specifier: string) =>
        new Promise((resolve) =>
          matchPath(specifier, undefined, undefined, extensions, (_, path) => resolve(fromNullable(path))),
        )
    : async () => none
